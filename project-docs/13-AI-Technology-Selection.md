...
> ⚠️ **本文档为 V1.2 / V2.0 前瞻规划。当前后端 MVP 未实现任何 AI 功能。** 文档记录技术选型的对比分析和最终决策，供 V1.2 启动时参考。
...
| 向量检索性能 | ★★★★★ | ★★★☆☆ | ★★★★★ |
| Java SDK 成熟度 | ★★★★☆ | ★★★★★ | ★★★☆☆ |
| 已有基础设施 | 需新部署 | 🟡 search-service 为骨架（V1.1 启用 ES） | 需新部署 |
| 混合检索（向量+关键词） | ★★★★★ | ★★★★☆ | ★★★☆☆ |
...
**V1.2 AI MVP 技术栈：ES 向量检索 + 自研 Agent + DeepSeek API（含 Embedding API）**
...
**关键规则：AI 服务通过 Feign 直连其他服务的 `/internal/` 接口（与现有 ia-api 客户端一致，内网调用绕过 Gateway）。**
